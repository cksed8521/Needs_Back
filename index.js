...
const { addUser, removeUser, getUser, getUsersInRoom } = require(__dirname +
  '/src/Chat/users')
...
app.use(
  '/get-categories-api',
  require(__dirname + '/src/backend-ms/categories')
)
...
io.on('connection', (socket) => {
  ...
  socket.on('sendMessage', async (message, callback) => {
    ...
    const chanel_sql =
      'INSERT INTO `channel_message`(`to_name`,`room` ,`message`,`timestamp`) VALUES (?,?,?,?)'
