...
router.post('/merchantsignup', upload.none(),  async (req, res) => {
    ...
    if (emailDuplicated.length){
        ...
        let new_merchant_id = parseInt(last_merchant_id[0].last_merchant.replace('M','')) + 1
         ...
         const merchantData = { merchant_id : new_merchant_id, brand_en_name: new_merchant_id}
         const [insertMerchantResult] = await db.query(sqlInsertMerchant, merchantData )
        ...
        const [insertContactResult] = await db.query(sqlInsertMerchantContact, submitData )
        ...
        let sqlUpdatedLastMerchantId = "UPDATE last_id SET last_merchant = ?, last_merchant_contact= ? WHERE id = 1"
        const [updateIdResult] = await db.query(sqlUpdatedLastMerchantId, [new_merchant_id, new_merchant_contact_id])
