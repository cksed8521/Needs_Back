...
router.get("/deliverytablefordashboard", (req, res) => {
  db.query("SELECT *, order_deliveries.status AS delivery_status, orders.status AS status FROM orders JOIN order_deliveries ON orders.delivery_id = order_deliveries.id WHERE merchant_id = 12 ORDER BY orders.created_at DESC LIMIT 7").then(([result]) => {
    res.json(result);
  });
});

router.get("/deliverystatusamount", (req, res) => {
  db.query("SELECT order_deliveries.status AS delivery_status FROM orders JOIN order_deliveries ON orders.delivery_id = order_deliveries.id WHERE merchant_id = 12 ORDER BY orders.created_at DESC").then(([result]) => {
    res.json(result);
  });
});
