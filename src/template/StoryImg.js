...
const upload = require(__dirname + "/StoryImgUploadmodule");
...
upload.single("image"),async (req, res) => {
  console.log("req", req.file.storyImg);
  console.log("req", req.file);
  const filename =  req.file.storyImg
  console.log('filename',filename)
  
  let sql =
    "UPDATE `brand_info` SET `brandStory_img` = ? WHERE `brand_info`.`merchant_id` = ?;"
  
  const [results1] = await db.query(sql, [req.file.storyImg , 12]);
