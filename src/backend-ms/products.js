...
async function getListData(req) {
  ...
  if (!req.query.searchInp) {
    ...
    switch (req.query.filter) {
      ...
      case 'launched':
        let sqlMerchantIdLaunched =
          "SELECT COUNT(1) totalLaunchedRows from products AS A WHERE A.merchant_id = ? AND A.launch_date <= NOW() AND A.launch_date != '0000-00-00'"
        const [[{ totalLaunchedRows }]] = await db.query(
        ...
        if (totalLaunchedRows > 0) {
          ...
          let sqlGetMerchantData = ` SELECT A.id, A.title, C.name as categories_name, A.outline, A.description, 
                                    ...
                                    WHERE A.merchant_id = ? AND A.launch_date < now() AND A.launch_date != '0000-00-00'
    ...
    switch (req.query.filter) {
      //launched
      case 'launched':
        let sqlMerchantIdLaunched = `SELECT COUNT(1) totalLaunchedRows from products AS A
          LEFT JOIN product_categories AS C ON A.categories_id = C.id
          WHERE A.merchant_id = ? AND A.launch_date <= NOW() AND A.launch_date != '0000-00-00'
          AND ?? LIKE ?`
        const [[{ totalLaunchedRows }]] = await db.query(
        ...
        if (totalLaunchedRows > 0) {
          ...
          const [results] = await db.query(sqlGetMerchantData, [
            req.query.id,
            `${searchTypeMap[req.query.searchType]}`,
            `%${req.query.searchInp}%`,
          ])
      ...
      //soldout
      case 'soldout':
        let sqlMerchantIdSoldOut = `SELECT COUNT(1) totalSoldOutRows from products AS A 
          LEFT JOIN product_skus AS B ON A.id = B.product_id 
          LEFT JOIN product_categories AS C ON A.categories_id = C.id
          WHERE A.merchant_id = ? AND B.stocks = 0
          AND ?? LIKE ?`
        const [[{ totalSoldOutRows }]] = await db.query(sqlMerchantIdSoldOut, [
          req.query.id,
          `${searchTypeMap[req.query.searchType]}`,
      ...
      //unlaunched
      case 'unlaunched':
        let sqlMerchantIdUnlaunched = `SELECT COUNT(1) totalUnlaunchedRows from products AS A
          LEFT JOIN product_categories AS C ON A.categories_id = C.id
          WHERE A.merchant_id = ? AND A.launch_date > NOW()
          AND ?? LIKE ?`
        const [
          [{ totalUnlaunchedRows }],
        ] = await db.query(sqlMerchantIdUnlaunched, [
          req.query.id,
          `${searchTypeMap[req.query.searchType]}`,
          `%${req.query.searchInp}%`,
        ])
      ...
      default:
        let sqlMerchantId =
          `SELECT COUNT(1) totalRows from products A 
           LEFT JOIN product_categories AS C ON A.categories_id = C.id
           WHERE A.merchant_id = ? AND ?? LIKE ?`
        const [[{ totalRows }]] = await db.query(sqlMerchantId, [
          req.query.id,
          `${searchTypeMap[req.query.searchType]}`,
          `%${req.query.searchInp}%`,
        ])
...
//PUT
router.put('/unlaunch', async (req, res) => {
  const output = {
    affectedRows: 0,
    success: false,
  }

  // const sqlUnLaunch = "UPDATE products SET launch_date = SUBDATE(NOW(),1) WHERE id = ?"
  const sqlUnLaunch = 'UPDATE products SET launch_date = NULL WHERE id = ?'
  const [results] = await db.query(sqlUnLaunch, req.query.id)

  if (results.affectedRows === 0) {
    return res.json({
      error: 'Update launch date failed',
      success: false,
    })
  }
