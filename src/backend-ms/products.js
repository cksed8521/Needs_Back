...
async function getListData (req) {
    ...
    let sqlMerchantIdLaunched = "SELECT COUNT(1) totalLaunchedRows from products AS A WHERE A.merchant_id = ? AND A.launch_date <= NOW()"
    const [[{ totalLaunchedRows }]] = await db.query(sqlMerchantIdLaunched,[req.query.id])
   ...
   return output;
    ...
    let sqlMerchantIdSoldOut = "SELECT COUNT(1) totalSoldOutRows from products AS A  LEFT JOIN  product_skus AS B ON A.id = B.product_id WHERE A.merchant_id = ? AND B.stocks = 0"
    const [[{ totalSoldOutRows }]] = await db.query(sqlMerchantIdSoldOut,[req.query.id])
   ...
   case 'unlaunched':
    ...
    let sqlMerchantIdUnlaunched = "SELECT COUNT(1) totalUnlaunchedRows from products AS A WHERE A.merchant_id = ? AND A.launch_date > NOW()"
    const [[{ totalUnlaunchedRows }]] = await db.query(sqlMerchantIdUnlaunched,[req.query.id])
   ...
   return output;
    ...
    default:
    let sqlMerchantId = "SELECT COUNT(1) totalRows from products WHERE merchant_id = ?"
